...
from .production_contract import validate_investment_result, ProductionContractError
...
class ResearchService:
    ...
    """
    def __init__(self, report_dir='data/ui_research', state_dir='data/ui_research/jobs', notifier=None, account_size_provider=None):
        ...
        self.account_size_provider = account_size_provider
    ...
    def _write_state(self, symbol, **data):
        ...
        self._atomic_json_write(self._state_path(symbol), payload)
        return payload
    ...
    def _run(self, symbol, lock):
        with lock:
            ...
            try:
                ...
                # Run the sovereign deterministic Decision/Validation/TradePlan layer too.
                # StockAnalyzer supplies evidence; BabyInvestmentSystem owns the research decision
                # and TradePlanAgent owns deterministic entry/invalidation/target scenarios.
                self._write_state(symbol,status='RUNNING',stage='Running deterministic decision, validation and trade plan',started_at=started)
                from investor.investment_system import BabyInvestmentSystem
                history=analyzer.provider.get_history(symbol,period='1y',interval='1d')
                history_rows=len(history) if history is not None else 0
                account_size=None
                if self.account_size_provider:
                    try: account_size=float(self.account_size_provider())
                    except Exception: account_size=None
                investment=BabyInvestmentSystem().evaluate(result,history=history,account_size=account_size)
                contract=validate_investment_result(investment, history_rows)
                combined={
                    'report':result,
                    'decision':investment.decision,
                    'validation':investment.validation,
                    'trade_plan':investment.trade_plan,
                    'external_provider':investment.external_provider,
                    'ai_scoring_authority':investment.ai_scoring_authority,
                    'ai_execution_authority':investment.execution_authority,
                ...
                ui=report_from_investment_result(symbol,combined)
                ui.raw={**(ui.raw or {}),'production_result':_jsonable(result),'investment_system':_jsonable(investment),'production_contract':contract.to_dict(),'history_observations':history_rows}
                target=self._report_path(symbol)
                ui_payload=ui.to_dict()
                ui_payload['research_completed_at']=datetime.now(timezone.utc).isoformat()
                self._atomic_json_write(target, ui_payload)
