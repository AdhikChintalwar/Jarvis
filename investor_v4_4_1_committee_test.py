...
class FakeClient:
    def reason_json(self, **kwargs):
        ...
        assert kwargs["task_name"] == "investment_committee_fast_v4_4_1"
        for candidate in kwargs["payload"]["candidates"]:
            assert "compact_evidence" in candidate
            assert "evidence_registry" not in candidate
            assert len(candidate["compact_evidence"]) < 80
...
print("V4.4.1 fast-committee governance contract: PASS")
