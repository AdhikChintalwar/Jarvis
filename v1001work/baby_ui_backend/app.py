...
def v10_full_flow(symbol:str):
    symbol=symbol.upper()
    p=REPORT_DIR/f'{symbol}.json'
    if not p.exists(): raise HTTPException(404,'Run Baby research before requesting the V10 full flow.')
    q=quote_service.get(symbol).__dict__
    portfolio=_paper_snapshot_with_quotes()
    proposal=proposal_service.build(symbol,json.loads(p.read_text()),q,portfolio)
    return v10_intelligence.full_flow(symbol,quote=q,portfolio=portfolio,proposal=proposal)

@app.get('/api/v10/research/{symbol}/exit-policy')
def v10_exit_policy(symbol:str):
    symbol=symbol.upper(); r=v10_intelligence.load(symbol)
    if r.get('status')=='NOT_RESEARCHED': raise HTTPException(404,'Run Baby research before requesting an exit policy.')
    q=quote_service.get(symbol).__dict__
    return v10_intelligence.exit_policy({**r,'quote':q})
