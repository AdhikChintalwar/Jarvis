...
class BaseProvider:
    ...
    def status(self):
        configured=self.configured(); circuit=self.circuit_open()
        if not configured: health='NOT_CONFIGURED'
        elif circuit or self.state.last_error: health='UNHEALTHY'
        elif self.state.last_success_at is not None: health='HEALTHY'
        else: health='UNVERIFIED'
        return {'provider':self.name,'configured':configured,'healthy':health=='HEALTHY','health':health,'eligible':configured and not circuit,
                'model':self.model(),'endpoint':self.endpoint(),'circuit_open':circuit,
                'circuit_retry_after':self.state.open_until if circuit else None,'last_error':self.state.last_error,
...
class OpenAIProvider(BaseProvider):
    ...
    def chat(self,messages,max_tokens=900,temperature=.1):
        ...
        try:
            ...
            if e.code==429 and any(x in body.lower() for x in ('insufficient_quota','credit_balance_exhausted','no credits remaining')):
