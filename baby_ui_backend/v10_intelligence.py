...
def canonical_market_snapshot(symbol:str, research:dict[str,Any], quote:dict[str,Any]|None)->dict[str,Any]:
    ...
    return {
        ...
        'execution_eligible':bool(quote.get('execution_eligible')),
        'execution_grade':quote.get('execution_grade') or 'UNKNOWN',
        'quote_age_seconds':quote.get('age_seconds'),
        'spread_percent':quote.get('spread_percent'),
        'validation_issues':quote.get('validation_issues') or [],
        'authority':quote.get('authority') or 'UNKNOWN',
...
class V10IntelligenceService:
    ...
    def exit_policy(self,research:dict[str,Any], snapshot:dict[str,Any]|None=None, setup_override:str|None=None)->dict[str,Any]:
        trade=research.get('trade_plan') or {}; current=trade.get('current_setup') or {}
        setup=_up(setup_override or current.get('status') or trade.get('status')); block={}
    ...
    def full_flow(self,symbol:str, quote:dict|None=None, portfolio:dict|None=None, proposal:dict|None=None)->dict[str,Any]:
        ...
        trade=r.get('trade_plan') or {}; research_current=trade.get('current_setup') or {}
        snapshot=canonical_market_snapshot(symbol,r,quote)
        effective_setup=_up((proposal or {}).get('setup_status') or research_current.get('status') or trade.get('status'))
        current=dict(research_current)
        current['research_status']=_up(research_current.get('status') or trade.get('status'))
        current['status']=effective_setup
        if effective_setup != current['research_status']:
            current['reason']='Current execution/reference snapshot no longer satisfies the persisted research setup.'
        ...
        flow={
          ...
          'trade_plan':trade,'current_setup':current,'exit_policy':self.exit_policy(r,snapshot,effective_setup),'paper_proposal':proposal,'portfolio':portfolio_view,
        ...
        if proposal and proposal.get('status')=='RESEARCH_SETUP_ONLY': flow['warnings'].append('A research setup exists, but current execution-quality market evidence is insufficient for a paper proposal.')
        if proposal and proposal.get('trade_quality_status')=='BLOCKED': flow['warnings'].append('The active setup fails the configured deterministic risk/reward quality gate.')
        return flow
