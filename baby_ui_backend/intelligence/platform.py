...
class V106IntelligencePlatform:
 ...
 def build(self,symbol,research,*,news=None,market_context=None,profile=None,options=None,institutional=None,flow=None):
  ...
  asset_type=str((profile or {}).get('asset_type') or (profile or {}).get('quote_type') or '').upper()
  if asset_type=='ETF':
   facts=None; warnings.append('SEC Company Facts intentionally skipped for ETF asset type.')
  else:
   try:facts=self.sec.companyfacts(symbol)
   except Exception as e:facts=None; warnings.append(f'SEC Company Facts unavailable: {e}')
  fundamentals=build_fundamentals(symbol,facts,not_applicable_reason='NOT_APPLICABLE_ETF: operating-company SEC Company Facts are intentionally skipped for ETF asset type.' if asset_type=='ETF' else None)
  ...
  return {'symbol':symbol.upper(),'status':'READY','schema_version':'10.6','release_version':'10.6.2','stages':stages,'warnings':warnings,
