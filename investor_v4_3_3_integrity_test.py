...
assert pd.Timestamp(analytics.common_start) == cutoff
assert pd.Timestamp(analytics.common_end) == pd.Timestamp(full.index[-1])
assert analytics.observations == int(full.loc[full.index >= cutoff].shape[0])
assert bt.actual_calendar_years < 2.0, bt.actual_calendar_years
assert bt.start == analytics.common_start, (bt.start, analytics.common_start)
assert bt.end == analytics.common_end, (bt.end, analytics.common_end)
assert "T" in bt.start
...
print("V4.3.3 backtest-integrity contract: PASS")
